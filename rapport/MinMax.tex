...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\WHILE {$Q \neq \emptyset$}
			...
			\ELSE
				\IF{($s \in V_{Min} \cup \{ Goal \}$) ou ($s.nbrSucc = 1$)}
...
\begin{center}
	\begin{tabular}{|l|c|}
		...
		\hline
		\textsc{Extraire-Min()} & $\mathcal{O}(\log n)$\\
		\hline
		\textsc{Insérer}(item) & $\mathcal{O}(\log n)$\\
		\hline
		\textsc{Augmenter-Clef}($clef,valeur$) & $\mathcal{O}(\log n)$\\
		\hline
		\textsc{Décrémenter-Clef}($clef,valeur$) & $\mathcal{O}(\log n)$\\
		\hline
...
\begin{enumerate}
	...
	\item \textsc{Relaxer}$(p, s, w)$ : Les instructions~\refrangeconj{lalgo:relaxer1}{lalgo:relaxer4} sont effectuées en $\mathcal{O}(1)$ tandis que celles~\refrangeconj{lalgo:relaxer5}{lalgo:relaxer10} sont en $\mathcal{O}(\log n)$ à cause de l'instruction \textsc{Décrémenter-Clef}. Enfin, les instructions~\refrangeconj{lalgo:relaxer11}{lalgo:relaxer12} ont une complexité en $\mathcal{O}(\log n)$. Dès lors, la complexité de cet algorithme secondaire est en $\mathcal{O}(\log n)$.
	
	 \item \textsc {Bloquer-Max}$(s)$: Les instructions~\ref{lalgo:bloc1} et~\ref{lalgo:bloc2} sont en $\mathcal{O}(\log n)$ tandis que les autres sont en $\mathcal{O}(1)$. Dès lors la complexité de cet algorithme est en $\mathcal{O}(\log n)$
