...
\end{defi}
\begin{rem}
	Même si par définition, considérer une fonction de poids à valeurs réelles est correct, dans notre cas nous ne nous intéresserons qu'à des fonctions de poids à valeurs dans $\mathbb{N}_0$. \todo{Ok ou je considère vraiment $\mathbb{R}^+_0 $?}
\end{rem}
...
\end{propriete}
\todo{Mettre une ref ou faire la preuve}
...
\begin{itemize}
	...
	\item[$\bullet$] $E_i$ est l'ensemble des arêtes du graphe $G$ duquel on a enlevé tous les arcs sortant d'un noeud Max et qu'on ne choisit pas d'emprunter car il existe un meilleur choix et ce après la i\up{ème} itération. Un tel arc est appelé un \emph{arc bloqué}.
