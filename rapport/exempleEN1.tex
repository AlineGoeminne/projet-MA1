...
\noindent	(rem: ces deux stratégies sont des stratégies sans mémoire), $(\sigma _{1}, \sigma _{2})$ est un équilibre de Nash de $(\mathcal{G}, v_{0})$. De plus, soit $\rho = \langle \sigma _{1},\sigma _{2} \rangle_{v_0}$ nous avons : $Cost_{1}(\rho) = 3$ et $Cost_{2}(\rho) = 0$.\\
	Montrons que $J_1$ ne possède pas de déviations profitables. Nous devrions pour cela le montrer pour toutes les stratégies possibles pour $J_1$. Nous nous contentons de le montrer pour les stratégies sans mémoire. \\
	 Considérons $\tilde{\sigma _{1}}(v) = $ $\begin{cases}
						...
						v_{3} & \text{ si } v = v_4						
						\end{cases}$ .\\
Dans ce cas, nous avons : \mbox{$Cost_{1}(\langle \tilde{\sigma _{1}},\sigma _{2} \rangle_{v_0}) = + \infty$}. C'est en fait le cas pour toutes les déviations $\tilde{\sigma_1}$ telles que $\tilde{\sigma_1}(v_1) = v_0$.  Ce ne sont donc pas des déviations profitables.\\
Considérons maintenant $\tilde{\sigma _{1}}(v) = $ $\begin{cases}
					...
					v_{2} & \text{ si } v = v_4						
					\end{cases}$.\\
On a alors \mbox{$Cost_{1}(\langle \tilde{\sigma _{1}},\sigma _{2} \rangle_{v_0}) = 3 = Cost_1(\langle \sigma _{1},\sigma _{2}\rangle_{v_0})$}. Ce n'est donc pas non plus une déviation profitable.
...
Nous avons ainsi testés toutes les déviations sans mémoire pour le $J_1$.
